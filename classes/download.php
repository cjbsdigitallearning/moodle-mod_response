...
class download {
    ...
    public function __construct(
        ...
        // Export dir/filename.
        $this->responsename = 'response_' . $context->instanceid . '_' . $this->get_file_date();
        $tempcoursedir = clean_param("response_{$response->course}", PARAM_FILE) . '_' . $this->get_file_date();
        $this->tempcoursedir = make_temp_directory($tempcoursedir);
        $this->tempresponsedir = make_temp_directory("{$tempcoursedir}/{$this->responsename}");
        // CSV headings.
    ...
    public function get_standard_response_values(stdClass $responsedata): array {
        ...
        $timemodified = '';
        if (isset($responsedata->response) && $responsedata->response->timemodified > 0) {
    ...
    public function get_user_response_fields(): array {
        ...
        foreach ($subfields as $subfield) {
            $responsefields[$subfield] = $subfield;
        }
     ...
     *
     * @param bool $allresponses Are we downloading all the response modules.
     * @param string $courseshortname Course shortname for the filename.
     */
    public function do_the_zip(bool $allresponses, string $courseshortname): void {

        if ($allresponses) {
            $time = $this->get_file_date();
            $filename = "responses_{$courseshortname}_{$time}.zip";
    ...
    /**
     * Time format for the zip filename.
     *
     * @return string The time format
     */
    private function get_file_date(): string {
        return userdate(time(), get_string('backupnameformat', 'langconfig'));
    }
}
