...
$PAGE->set_url('/mod/response/deleteanswer.php', ['id' => $cm->id, 'u' => $userid]);
...
$response->user_responses = $instance->load_response_for_users($response, [$userid], true, true);

if (empty($response->user_responses[$userid])) {
    // There's no response from this user.
    throw new moodle_exception('deleteresponsenotcomplete', 'response');
}
...
if ($confirm && confirm_sesskey()) {
    ...
    redirect(new moodle_url('/course/view.php', ['id' => $cm->course], 'module-' . $cm->id));
}
...
$deleteurl = new moodle_url('/mod/response/deleteanswer.php', ['id' => $id, 'u' => $userid, 'confirm' => 1]);
$cancelurl = new moodle_url('/mod/response/view.php', ['id' => $id]);
