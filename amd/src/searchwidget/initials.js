...
/**
 * A small dropdown to filter responses.
 *
 * Copied and altered from grade/amd/src/searchwidget/initials.js
 *
 * @module    mod_response/searchwidget/initials
 ...
 * @param {Null|String} search The search value to filter by.
 */
export const init = (callingLink, search = null) => {
    if (registered) {
        return;
    }
    const pendingPromise = new Pending();
    registerListenerEvents(callingLink, search);
 ...
 * @param {Null|String} search The search value to filter by.
 */
const registerListenerEvents = (callingLink, search = null) => {
    const events = [
        'click',
    ...
    events.forEach((event) => {
        document.addEventListener(event, (e) => {
            ...
            let {firstActive, lastActive, ifirst, ilast} = onClickVariables();
            let itemToReset = '';
            ...
            if (e.target.closest(`${selectors.formDropdown}`) && e.target.type === selectors.formItems.type) {
                if (e.target.dataset.action === selectors.formItems.save) {
                    ...
                    const params = {
                        'id': e.target.closest(selectors.formDropdown).dataset.cmid,
                        'ifirst': ifirst.parentElement.classList.contains('initialbarall') ? '' : ifirst.value,
                        'ilast': ilast.parentElement.classList.contains('initialbarall') ? '' : ilast.value,
                    };
                    if (search !== null) {
 ...
 *
 * @returns {{firstActive: HTMLElement, lastActive: HTMLElement, ifirst: ?String, ilast: ?String}}
 */
const onClickVariables = () => {
    ...
    let ifirst = firstActive.querySelector(selectors.pageClickableItem);
    let ilast = lastActive.querySelector(selectors.pageClickableItem);
    return {firstActive, lastActive, ifirst, ilast};
};
