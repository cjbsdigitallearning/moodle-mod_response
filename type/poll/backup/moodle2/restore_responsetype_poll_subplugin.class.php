...
/**
 * Defines the steps necessary to restore a poll response.
 *
...
/**
 * Structure step to restore a poll activity.
 *
 * @package     responsetype_poll
 * @category    backup
 * @copyright   2017 Peter Spicer <peter.spicer@catalyst-eu.net>
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_responsetype_poll_subplugin extends restore_subplugin {

    /**
     * Defines the overall structure of the response data for poll responses.
     *
    ...
    protected function define_response_subplugin_structure() {
        ...
        $elementpath = $this->get_pathfor('/responsetype_poll_settings');

        $paths[] = new restore_path_element($element, $elementpath);

        $element = $this->get_namefor('choices');
        $elementpath = $this->get_pathfor('/responsetype_poll_choices');

        $paths[] = new restore_path_element($element, $elementpath);

        if ($userinfo) {
            $element = $this->get_namefor('answers');
            $elementpath = $this->get_pathfor('/responsetype_poll_answers');
            $paths[] = new restore_path_element($element, $elementpath);
        }
        return $paths;
    }
    ...
    /**
     * Store the general poll settings from the XML into the DB.
     *
     * @param array $data The items from /activity/response in the backup XML.
     */
    public function process_responsetype_poll_settings($data) {
        global $DB;

        $data = (object) $data;
        $data->response = $this->get_new_parentid('response');

        $DB->insert_record('responsetype_poll', $data);
    }

    /**
     * Store the poll choices from the XML into the DB.
     *
     * @param array $data The items from /activity/response in the backup XML.
     */
    public function process_responsetype_poll_choices($data) {
        global $DB;

        $data = (object) $data;
        $data->response = $this->get_new_parentid('response');

        $DB->insert_record('responsetype_poll_choice', $data);
    }

    /**
     * Store the user's answers from the XML into the DB.
     *
     * @param array $data The items from /activity/response in the backup XML.
     */
    public function process_responsetype_poll_answers($data) {
        global $DB;
        ...
        $new->choice = $data['choice'];
        $new->reflection_text = $data['reflection_text'];

        $DB->insert_record('responsetype_poll_user', $new);
    }
    ...
    public function after_execute_response() {
        ...
        $result = $DB->get_records('responsetype_poll_user', array('response' => $parentid));
